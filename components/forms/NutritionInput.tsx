...
import { Utensils, Refrigerator, Zap, TrendingUp, TrendingDown, Leaf, Plus, X, Loader2, Beef, Carrot, Egg, Droplets, Nut, Activity } from 'lucide-react';
    ...
    return (
        <div className="space-y-6 animate-fade-in pb-20">
          ...
          `}
                ...
                {/* --- NEW: INTENSITY & HEALTH --- */}
                <div className="grid grid-cols-2 gap-4">
                    {/* Intensity Selector */}
                    <div className="p-4 rounded-xl border border-white/10 bg-black/20">
                        <label className="flex items-center gap-2 text-sm text-gray-400 mb-3">
                            <TrendingUp className="w-4 h-4 text-orange-400" /> Cường độ tập
                        </label>
                        <div className="flex gap-1 bg-black/40 p-1 rounded-lg">
                            {(['low', 'medium', 'hard'] as const).map((level) => (
                                <button
                                    key={level}
                                    onClick={() => setUserData({ ...userData, selectedIntensity: level })}
                                    className={`flex-1 py-1.5 rounded-md text-xs font-bold capitalize transition-all ${userData.selectedIntensity === level
                                        ? level === 'hard' ? 'bg-red-500 text-white' : level === 'medium' ? 'bg-orange-500 text-white' : 'bg-green-500 text-white'
                    ...
                    <div className="p-4 rounded-xl border border-white/10 bg-black/20">
                        <label className="flex items-center gap-2 text-sm text-gray-400 mb-3">
                            <Activity className="w-4 h-4 text-pink-400" /> Sức khỏe
                        </label>
                        <select
                            value={userData.healthCondition || 'Good'}
                            onChange={(e) => setUserData({ ...userData, healthCondition: e.target.value as any })}
                            className="w-full bg-black/40 text-white text-sm rounded-lg p-2 border border-white/10 focus:outline-none focus:border-cyan-500"
                        >
                            <option value="Good">💪 Sung sức</option>
